...
- **Staging:** AXP sandboxes get fixtures via gzipped `setup` in the YAML (`sync-experiment-fixture-setup.py`); graders read `/workspace/fixtures` through `eval_bootstrap.py`.
- **AXP pin:** `.axp-version` → `0.3.3-rp`; layout 2.0 for `axp upload`.
- **Run:** `op run --env-file=.env -- axp run experiments/...yaml -n 1 -j 5`
- **Known flakes:** Docker port-8800 binding; axp-base image pull timeout under high `-j` — pre-pull `514labs/axp-base:0.3.3-rp`.
